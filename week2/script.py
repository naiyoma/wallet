...
def generate_new_address(redeem_script):
    ...
    addr = Address(redeem_script.serialize().hex(), encoding='bech32', script_type='p2sh')
...
def create_send_transaction(addr):
    ...
    #First Initate a Transaction Class
    #get a an unspend transaction id to be used as previous output and current input
    #create an input using the previous transaction and the created private key
...
def spend_transaction(t, private_key):
    ...
    wallet = Wallet.create("test5_wallet")
    change_address = wallet.new_key().address
    main_output_amount = 5 
    change_output_amount = utxo_value - main_output_amount

    new_tx = Transaction()
    new_tx.add_input(prev_txid=t.txid, output_n=0, unlocking_script=redeem_script.serialize())  
    new_tx.add_output(main_output_amount, addr.address)  
    new_tx.add_output(change_output_amount, change_address)

    new_tx.sign(new_tx.sign(private_key))
